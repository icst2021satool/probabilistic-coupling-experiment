...
2. `<Class name>`.**sub.json**. This file lists the leaves of the reduced subsumption graph for each method of (see in the paper the description of the reduced subsumption graph). For each leave of the method (refered to as S`<number>`) there is a list of id numbers of DUAs subsumed.
